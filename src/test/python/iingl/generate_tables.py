...
CUMULATIVE_TABLE_KEYS = [
    ("solveTimeCumulative", "Solve Time Cumulative (s)"),
]
...
def latex_header(cumulative=False):
    if not cumulative:
        ...
        columns = len(MAIN_TABLE_KEYS) * 2 + 2 + (1 if INCLUDE_SPEEDUP else 0)

        tabular = "c|" + "|".join(
            ["cc"] * len(MAIN_TABLE_KEYS)
        )
...
"""
        ...
        if INCLUDE_SPEEDUP:
            groups.append(r"\multirow{2}{*}{\centering Speedup}")
        ...
        for _ in MAIN_TABLE_KEYS:
            subheaders.extend(["Chain", "One-by-One"])
        if INCLUDE_SPEEDUP:
            subheaders.append("")
        ...
        if INCLUDE_CUMULATIVE_SPEEDUP:
            groups.append(r"\multirow{2}{*}{\centering Speedup Cumulative}")
...
def generate_cumulative_table(problem, data):
    ...
    for i, diff in enumerate(DIFFS):
        ...
        if INCLUDE_CUMULATIVE_SPEEDUP:
            values.append(data["speedupCumulative"][i])
...
def generate_flat_status_table(data):
    ...
    lines.append(
        ...
        r"\multicolumn{4}{c}{Status One-by-one}\\"
    )
